...
assert.match(checkoutPricing, /USPS_GROUND_ADVANTAGE/);
assert.match(checkoutPricing, /PRIORITY_MAIL/);
assert.match(checkoutPricing, /usps_health/);
...
assert.match(confirmation, /Payment received\. Your order has been received and is being reviewed for fulfillment\./);
assert.match(confirmation, /You’ll receive updates when your order moves into production and when tracking is available\./);
assert.match(confirmation, /order\?\.payment_status !== 'paid' && settings\.payment_notes_customer/);
assert.doesNotMatch(confirmation, /Your payment has been successfully processed\. Your order is now being prepared for fulfillment\./);
