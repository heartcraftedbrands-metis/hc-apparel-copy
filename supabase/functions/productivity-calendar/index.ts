...
async function calendars(row: Record<string, string>) {
  const response = await fetch('https://www.googleapis.com/calendar/v3/users/me/calendarList?maxResults=250', { headers: { authorization: `Bearer ${await access(row)}` } });
  if (!response.ok) bad('Could not list Google calendars. Reconnect the account.', 502);
  const result = await response.json();
  return (result.items || []).map((item: Record<string, unknown>) => ({ id: item.id, name: item.summary, access_role: item.accessRole, primary: Boolean(item.primary) }));
}
...
async function suggestions(userId: string) {
  ...
  const staffByName = new Map((staff.data || []).map((member: { name: string; id: string }) => [member.name, member.id]));
  const records = (parsed!.suggestions || []).slice(0, 5).flatMap(item => {
    ...
    return [{ created_by: userId, title: limited(item.title, 140), description: limited(item.description, 1000), reason: limited(item.reason, 400), assigned_to: staffByName.get(String(item.assigned_to)) || null,
  ...
  const enriched = await Promise.all(records.map(async record => {
    ...
    if (record.related_order_id) {
      const { data: order } = await db.from('orders').select('customer_name,status,fulfillment_status,order_items').eq('id', record.related_order_id).single();
  ...
  const { data, error } = await db.from('calendar_event_suggestions').insert(enriched).select('id,title,description,reason,assigned_to,priority,status,scheduled_start,scheduled_end,related_order_id,related_quote_id,related_vendor_draft_id');
...
Deno.serve(async req => {
  ...
  try {
    ...
    if (action === 'status') {
      const { data } = await db.from('calendar_connections').select('google_email,selected_calendar_id,selected_calendar_name,connected_at').eq('owner_user_id', user.id).maybeSingle();
    ...
    if (action === 'start') {
      ...
      for (const [key, value] of Object.entries({ client_id: env('GOOGLE_CLIENT_ID'), redirect_uri: FN, response_type: 'code', scope: 'openid email https://www.googleapis.com/auth/calendar.calendarlist.readonly https://www.googleapis.com/auth/calendar.events', access_type: 'offline', prompt: 'consent select_account', login_hint: OWNER, state })) target.searchParams.set(key, value);
    ...
    if (action === 'create_event') {
      ...
      const row = await connection(user.id);
      if (!row.selected_calendar_id) bad('Select a writable calendar first.');
      ...
      const { error: savedError } = await db.from('created_calendar_events').insert({ suggestion_id: id, created_by: user.id, assigned_to: suggestion.assigned_to, related_order_id: suggestion.related_order_id, related_quote_id: suggestion.related_quote_id, related_vendor_draft_id: suggestion.related_vendor_draft_id,
        title: suggestion.title, description: suggestion.description, priority: suggestion.priority, scheduled_start: start.toISOString(), scheduled_end: end.toISOString(), calendar_event_id: event.id, calendar_id: row.selected_calendar_id, event_url: event.htmlLink || null });
      ...
      await db.from('calendar_event_suggestions').update({ calendar_event_id: event.id, calendar_id: row.selected_calendar_id }).eq('id', id);
