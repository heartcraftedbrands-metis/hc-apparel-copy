...
export default function CreatePricingFromSSModal({ item, open, onClose, onSuccess }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const pricing = await base44.entities.VendorPricing.create({
        ...
        garment_brand: item.brand || '',
        garment_style_number: item.style_number || '',
        ...
        notes: form.notes || (item.sku ? `SKU: ${item.sku}` : ''),
        is_active: true,
      });
      await base44.entities.SSCatalogItem.update(item.id, { linked_vendor_pricing_id: pricing.id });
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Create Vendor Pricing Record</DialogTitle></DialogHeader>

        {/* Auto-filled product details (read-only) */}
        <div className="p-3 bg-muted/40 rounded-lg text-sm space-y-1.5 mb-2">
          <p className="text-xs text-muted-foreground uppercase tracking-wide font-semibold mb-1">Auto-filled from S&S Catalog</p>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
            <div><span className="text-muted-foreground">Brand:</span> <span className="font-medium">{item.brand || '—'}</span></div>
            <div><span className="text-muted-foreground">Style #:</span> <span className="font-medium">{item.style_number || '—'}</span></div>
            <div className="col-span-2"><span className="text-muted-foreground">Product:</span> <span className="font-medium">{item.product_name}</span></div>
            <div><span className="text-muted-foreground">SKU:</span> <span className="font-medium">{item.sku || '—'}</span></div>
            <div><span className="text-muted-foreground">Category:</span> <span className="font-medium">{item.product_category || '—'}</span></div>
            <div><span className="text-muted-foreground">Blank Cost:</span> <span className="font-semibold text-primary">${item.blank_cost?.toFixed(2) || '0.00'}</span></div>
          </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <select className={selectClass} value={form.print_method} onChange={e => setForm(f => ({ ...f, print_method: e.target.value }))}>
