    ...
    <section className="bg-[#f6f3e9] py-14 md:py-20" aria-labelledby="homepage-specials-title">
      <div className="container mx-auto px-4">
        <div className="mb-8 max-w-2xl">
          <p className="mb-2 text-xs font-bold uppercase tracking-[0.24em] text-[#9d7b35]">S&amp;S SALE PICKS</p>
          ...
          <p className="mt-2 text-[#586251]">
            Current S&amp;S sale styles available through HC Apparel while supplies last.
          </p>
        </div>
          ...
          {specials.map(item => (
            <article key={item.id} className="overflow-hidden rounded-2xl border border-[#d9d4c4] bg-white shadow-sm sm:grid sm:grid-cols-[43%_1fr]">
              ...
              <div className="flex flex-col justify-center p-5 md:p-7">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-[#6a735c]">{item.brand} · {item.style_number}</p>
                ...
                <p className="mt-5 text-xs font-bold uppercase tracking-wider text-[#6a735c]">HC Apparel Customer Price</p>
                <span className="mt-1 text-2xl font-extrabold text-[#26351f]">{money(item.price)}</span>
                {!isAuthenticated && <p className="mt-2 text-xs text-[#586251]">Sign in or create an account to get this price.</p>}
                <Link to={`/ProductDetail?id=${encodeURIComponent(item.product_id)}`} className="mt-5 inline-flex w-fit items-center justify-center rounded-lg bg-[#34472c] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#253720]">Shop Deal</Link>
              </div>
            </article>
          ))}
        </div>
