...
const ENTITY_CONFIG = {
  ...
  CheckoutFinancialSettings: { table: 'checkout_financial_settings' },
    ...
    if (error) {
      ...
      try {
        ...
        if (response?.json) {
          ...
          if (detail?.error) wrapped.message = detail.error;
          wrapped.code = detail?.code || wrapped.code;
        }
      } catch { /* Preserve the safe generic function error. */ }
      throw wrapped;
    }
    return { data };
  },
