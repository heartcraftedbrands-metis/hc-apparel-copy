...
export default function AdminBrandPages() {
  ...
  const vendorAction = async action => {
    ...
    else { setCatalogResult(data); setMessage(action === 'stage_brand_styles' ? `${data.staged_styles} ${page.name} styles staged privately. No products published.` : `${data.skus} SKU variants staged privately. Review before any product import or publication.`); }
  };
  return <main className="min-h-screen bg-[#f8f5ed] px-4 py-10"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-8 grid gap-5 md:grid-cols-[240px_1fr]">
      ...
      {page && <div className="min-w-0 rounded-2xl border bg-white p-5 shadow-sm sm:p-7">
        ...
        {['Comfort Colors', 'DRI DUCK'].includes(page.name) && <div className="mt-8 rounded-xl border border-amber-200 bg-amber-50 p-5"><h2 className="font-bold">Private S&S catalog staging</h2><p className="mt-1 text-sm text-muted-foreground">Fetch real styles and SKU variants for {page.name}. This does not create or publish products, change prices, or submit orders.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={() => vendorAction('stage_brand_styles')} className="rounded-lg border bg-white px-4 py-2 text-sm font-semibold disabled:opacity-50">Stage focused styles</button><button type="button" disabled={busy || !catalogResult?.import_session_id} onClick={() => vendorAction('sync_brand_products')} className="rounded-lg border bg-white px-4 py-2 text-sm font-semibold disabled:opacity-50">Fetch SKU details</button></div></div>}
      </div>}
    </div>
  </div></main>;
}
