...
export default function AdminSocialMediaStudio() {
  ...
  const generate = () => run('generate', async () => {
    ...
    await loadHistory();
    setNotice(form.image_mode === 'artwork' ? 'Your artwork and AI-generated copy are saved as a private HC Apparel draft.' : form.product_id && form.image_mode === 'product' ? 'AI copy and the real catalog product image are saved as a private draft.' : 'Image and AI copy are saved as a private draft. Review before Buffer handoff.');
  });
          ...
          <Card className="min-w-0 border-stone-200 shadow-sm">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="rounded-xl border bg-stone-50 p-4">
                ...
                <div className="mt-4"><SelectField label="Post image" value={form.image_mode} options={[["product", "Use Product Image (default)"], ["lifestyle", "Use AI Generated Image"], ["artwork", "Upload My Artwork"]]} onChange={value => set('image_mode', value)} /><p className="mt-1 text-xs text-muted-foreground">{form.image_mode === 'artwork' ? 'Your uploaded artwork is used as-is. OpenAI generates copy only.' : selectedProduct ? form.image_mode === 'product' ? 'Uses the selected product’s actual primary catalog image. OpenAI generates copy only.' : 'Generates a lifestyle image based on the real product and selected catalog color.' : 'Choose a product to use its real image. Without a product, an AI image is generated.'}</p></div>
            ...
            <Card className="border-stone-200 shadow-sm">
              ...
              <CardContent className="space-y-4">
                {post ? <>
                  ...
                  {post.image_mode === 'artwork' ? <p className="text-xs text-muted-foreground">Your uploaded artwork · saved with this draft</p> : post.image_mode === 'product' ? <p className="text-xs text-muted-foreground">Actual catalog product image · saved with this draft</p> : <p className="text-xs text-muted-foreground">AI-generated image · saved with this draft</p>}
