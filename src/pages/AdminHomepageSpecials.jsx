    ...
    <main className="min-h-screen bg-[#f6f3e9] p-4 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[['Requested brands', 3], ['Available qualifying S&S sale products', targetCandidates.filter(isCurrentSale).length], ['Blocked / replacement needed', targetCandidates.filter(row => !isCurrentSale(row) || !row.eligible).length], ['Active carousel slides', rows.filter(row => row.approved && row.active && row.promotion_slot).length]].map(([label, count]) => (
        ...
        <section className="rounded-2xl border bg-white p-4 md:p-5" aria-label="S&S Sale Promotions">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">{[1,2,3].map(slot => { const record = rows.find(row => row.active && row.promotion_slot === slot); const candidate = record && candidates.find(row => row.product_id === record.product_id && row.sku === record.sku); return <div key={slot} className="rounded-xl border bg-[#f6f3e9] p-4"><p className="text-xs font-bold uppercase tracking-widest text-[#9d7b35]">Slide {slot}</p>{candidate ? <div className="mt-2 space-y-1 text-sm"><p className="font-bold text-[#283820]">{candidate.brand}</p><p>{getPublicProductName({ name: candidate.product_name, brand: candidate.brand, category: candidate.category })}</p><p>Style: {candidate.style_number}</p><p>S&amp;S salePrice: <strong>{money(candidate.vendor_special_price)}</strong></p><p>S&amp;S sale status: <strong>{isCurrentSale(candidate) ? 'Current' : 'Sale promotion needs replacement.'}</strong></p><p>Source: {candidate.sale_field_source || 'Unavailable'}</p><p>Inventory: {candidate.inventory_qty > 0 ? `${candidate.inventory_qty} in stock` : 'Unavailable'}</p><p>HC Apparel Customer Price: <strong>{money(candidate.public_price)}</strong></p><p>Display order: {record.display_order}</p><p>Status: {record.active ? 'Active' : 'Inactive'}</p></div> : <p className="mt-2 text-sm text-amber-800">Sale promotion needs replacement.</p>}</div>; })}</div>
        </section>
