...
export default function AdminProductivityDashboard() {
  ...
  const { data: status } = useQuery({ queryKey: ['calendar-status'], queryFn: () => calendarAction('status'), retry: false });
  ...
  const edit = suggestion => { setEditing(suggestion.id); setFields({ title: suggestion.title, description: suggestion.description, priority: suggestion.priority, assigned_to: suggestion.assigned_to || '', scheduled_start: dateInput(suggestion.scheduled_start), scheduled_end: dateInput(suggestion.scheduled_end) }); setError(''); };
  const saveEdit = suggestion => action(async () => {
    ...
    const { error: saveError } = await supabase.from('calendar_event_suggestions').update({ title: fields.title.trim().slice(0, 140), description: (fields.description || '').trim().slice(0, 1000), priority: fields.priority, assigned_to: fields.assigned_to || null, scheduled_start: new Date(fields.scheduled_start).toISOString(), scheduled_end: new Date(fields.scheduled_end).toISOString(), due_date: new Date(fields.scheduled_start).toISOString(), status: 'suggested', updated_at: new Date().toISOString() }).eq('id', suggestion.id).in('status', ['suggested','approved']);
    if (saveError) throw saveError;
  ...
  const saveTask = suggestion => action(async () => {
    const { error: saveError } = await supabase.from('productivity_tasks').insert({ title: suggestion.title, description: suggestion.description, assigned_to: suggestion.assigned_to, priority: suggestion.priority, status: 'approved', due_date: suggestion.scheduled_start, related_order_id: suggestion.related_order_id, related_quote_id: suggestion.related_quote_id, related_vendor_draft_id: suggestion.related_vendor_draft_id });
    if (saveError) throw saveError;
  ...
  const createEvent = suggestion => action(async () => {
    if (!reviewed[suggestion.id] || !window.confirm(`Create ONE real event on ${status.connection.selected_calendar_name}? This will not email customers.`)) return;
  ...
  return <main className="mx-auto max-w-6xl space-y-8 px-4 py-10"><header><Link className="text-sm text-primary underline" to="/AdminDashboard">Admin Dashboard</Link><h1 className="mt-3 text-3xl font-bold">Productivity Dashboard</h1><p className="text-muted-foreground">Private HC Apparel tasks and calendar suggestions. Nothing is sent to Google without a separate, reviewed approval.</p></header>
    {error && <p role="alert" className="rounded-lg border border-red-200 bg-red-50 p-3 text-red-700">{error}</p>}{message && <p role="status" className="rounded-lg border bg-white p-3">{message}</p>}
    ...
    <section className="space-y-4 rounded-2xl border bg-[#f8f6ef] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">AI Schedule Assistant</h2><p className="text-sm text-muted-foreground">Uses real operational IDs, statuses and dates; no payment details or customer email is sent to OpenAI.</p></div><button className="rounded-lg bg-primary px-4 py-2 text-white disabled:opacity-50" disabled={busy || !status?.openai_configured} onClick={() => action(async () => { const result = await calendarAction('suggest'); setMessage(`${result.suggestions.length} private suggestions saved. No Google events created.`); })}>Generate suggestions</button></div>{!status?.openai_configured && <p className="text-sm">Configure OPENAI_API_KEY in Supabase to enable suggestions.</p>}{suggestionsError && <p role="alert">Could not load suggestions; deploy the productivity migration.</p>}
      {suggestions.filter(item => ['suggested','approved'].includes(item.status)).map(item => <article className="space-y-3 rounded-xl border bg-white p-5" key={item.id}><div className="flex flex-wrap justify-between gap-2"><h3 className="font-bold">{item.title}</h3><span className="text-xs uppercase text-muted-foreground">{item.priority} · {item.status}</span></div><p className="text-sm">{item.description}</p><p className="text-sm text-muted-foreground">{displayDate(item.scheduled_start)} – {displayDate(item.scheduled_end)} · {staffNames.get(item.assigned_to) || 'Unassigned'}</p><p className="text-xs text-muted-foreground">Reason: {item.reason || 'Operational follow-up'} · Reference: {item.related_order_id || item.related_quote_id || item.related_vendor_draft_id || 'general operations'}</p>
        {editing === item.id && <div className="grid gap-2 sm:grid-cols-2"><label className="text-sm">Title<input className="w-full rounded border p-2" value={fields.title} onChange={e => setFields({ ...fields, title: e.target.value })} /></label><label className="text-sm">Owner<select className="w-full rounded border p-2" value={fields.assigned_to} onChange={e => setFields({ ...fields, assigned_to: e.target.value })}><option value="">Unassigned</option>{team.map(person => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><label className="text-sm">Start<input className="w-full rounded border p-2" type="datetime-local" value={fields.scheduled_start} onChange={e => setFields({ ...fields, scheduled_start: e.target.value })} /></label><label className="text-sm">End<input className="w-full rounded border p-2" type="datetime-local" value={fields.scheduled_end} onChange={e => setFields({ ...fields, scheduled_end: e.target.value })} /></label><label className="text-sm">Priority<select className="w-full rounded border p-2" value={fields.priority} onChange={e => setFields({ ...fields, priority: e.target.value })}>{['low','normal','high','urgent'].map(priority => <option key={priority}>{priority}</option>)}</select></label><label className="text-sm sm:col-span-2">Description<textarea className="w-full rounded border p-2" value={fields.description} onChange={e => setFields({ ...fields, description: e.target.value })} /></label><button className="rounded border px-3 py-2" disabled={busy} onClick={() => saveEdit(item)}>Save edits</button></div>}
