...
import { CATEGORY_FILTERS, SORT_OPTIONS, filterAndSortGarments, getProductBrand } from '@/lib/shopGarmentFilters';
...
export default function BrandPage() {
  ...
  const categoryButtons = (page.categories || []).map(value => CATEGORY_FILTERS.find(item => item.value === value)).filter(Boolean);
  ...
  return <div className="min-h-screen bg-[#f8f5ed]">
    <section className="bg-[#303f20] text-[#f8f5ed]">
      <div className="container mx-auto grid min-w-0 md:grid-cols-2">
        <div className="h-64 min-w-0 sm:h-80 md:h-[430px]">
          <CatalogEditorialImage src={heroImage} alt={`${page.name} apparel blanks`} className="object-contain p-5" />
        </div>
        <div className="flex min-w-0 flex-col justify-center px-5 py-9 sm:px-10 md:py-14">
    ...
    <section id="brand-products" className="container mx-auto px-4 py-10 sm:py-14">
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        ...
        <select aria-label="Category" value={category} onChange={event => setCategory(event.target.value)} className="h-10 min-w-0 rounded-lg border bg-white px-3 text-sm"><option value="all">All categories</option>{CATEGORY_FILTERS.slice(1).map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
